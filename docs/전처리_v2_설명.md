...
### LightGBM, 5-fold 평균 (fold 컬럼 기준 GroupKFold, 최종본)

| | original | refined |
|---|---|---|
| ROC-AUC | 0.745082 ± 0.0009 | **0.748361 ± 0.0009 (+0.0033)** |
| F1 | 0.262997 | 0.264521 |
| Accuracy | 0.905215 | 0.905298 |

`coord_cluster_size` 추가 전까지는(파생 비율 3종 + 결측 대체 + 플래그만 있을 때) 사실상 동률이었음. `coord_cluster_size`를 추가하니 **5개 fold 전부에서 개별적으로 ROC-AUC가 상승**(노이즈 아님) — importance 5위(167.2)까지 올라옴. `industry_specialization_300m`/`dong_industry_count_growth`는 여전히 top-15에 들지만 트리가 이미 비슷한 분기를 만들 수 있어 이쪽은 추가 이득이 거의 없었음.
