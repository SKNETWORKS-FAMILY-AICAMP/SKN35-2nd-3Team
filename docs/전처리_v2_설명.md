...
### LightGBM, 5-fold 평균 (fold 컬럼 기준 GroupKFold)

| | original | refined |
|---|---|---|
| ROC-AUC | 0.7451 ± 0.0009 | 0.7451 ± 0.0010 |
| F1 | 0.2630 | 0.2630 |

사실상 동률. `industry_specialization_300m`(8위), `dong_industry_count_growth`(13위)는 실제로 쓰이지만, 트리 모델이 이미 원본 컬럼 조합으로 비슷한 분기를 만들 수 있어서 전체 성능엔 추가 이득 없음.
...
## ⚠️ 발견: `transitioned_next` 데이터 누수 의심 (미해결, 후속 조치 필요)
...
**미완료**: 이 문서 작성 시점 기준 아직 `preprocess_v2.py`에 반영 안 함 — 다음 작업으로 추가 필요.
