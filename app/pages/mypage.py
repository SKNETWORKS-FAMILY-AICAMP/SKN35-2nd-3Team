...
def get_owner_competition_density(store_id: str, snapshot_date: str) -> dict | None:
    """spatial_density_features(반경 300m 내 동일업종/전체 매장 수, 20m 내 클러스터
    크기, 가장 가까운 동일업종 매장까지 거리) — app.py의 _owner_competition_density()와
    같은 쿼리. app.py는 진입점 파일명 충돌 때문에 이 페이지에서 바로 import할 수
    없어서(모듈 상단 주석 참고) 독립적으로 둔다."""
    if not store_id or not snapshot_date:
        return None
    db = _load_db_module()
    engine = db.get_engine()
    if engine is None:
    ...
    sql = text(
        ...
        "nearest_same_industry_distance_m FROM spatial_density_features "
...
def _transition_counts_from(industry_code: str) -> dict:
    """industry_transitions(실제 전환 이력) 기준 목적지별 전환 건수 — app.py의
    동명 함수와 같은 쿼리."""
    db = _load_db_module()
    engine = db.get_engine()
    if engine is None:
    ...
    sql = text(
        "SELECT to_industry_code, COUNT(*) AS n FROM industry_transitions "
        "WHERE from_industry_code = :industry_code GROUP BY to_industry_code"
    )
    with engine.connect() as conn:
...
def _wilson_lower_bound(p: float, n: int, z: float = 1.96) -> float:
    """Wilson score interval 하한(95%) — app.py의 동명 함수와 같은 계산. 표본이
    적을수록 하한이 크게 깎여서, 표본 1~2건짜리가 우연히 100%를 찍고 업종전환
    추천 1위를 차지하는 걸 막아준다."""
    if n <= 0:
        return 0.0
    denom = 1 + z**2 / n
...
def get_industry_switch_recommendations(industry_code: str, top_n: int = 3) -> list[dict]:
    """업종전환 추천 — app.py의 _industry_switch_recommendations()와 동일 로직
    (Wilson 하한 정렬 + 모델링 제외 업종군 필터 + 실제 전환 건수 병기)."""
    ...
    sql = text(
        ...
        "s.survival_rate, s.sample_size FROM industry_survival_stats s "
        "JOIN industries i ON i.industry_code = s.to_industry_code "
        "WHERE s.from_industry_code = :industry_code AND s.to_industry_code != :industry_code"
    )
    with engine.connect() as conn:
        rows = conn.execute(sql, {"industry_code": industry_code}).mappings().all()
...
def init_session_state():
    ...
    # 로그아웃은 auth.logout()이 인증 관련 세션 키만 지우고, 이 페이지가 아래에서
    # "한 번만 조회하고 캐싱"해두는 display_name/predictions/store_status/
    # store_snapshots는 그대로 남겨둔다 — 그래서 같은 브라우저 탭에서 로그아웃 후
    # 다른 계정으로 다시 로그인하면 새 계정인데도 이전 계정의 캐시가 그대로 보이는
    # 문제가 있었다(2026-08-31 확인, "어떤 계정으로 로그인해도 같은 데이터가 보인다"
    # 는 제보와 일치). 캐시를 만들 때 같이 저장해 둔 사용자 id와 지금 로그인한
    # 사용자가 다르면, 그 캐시부터 지우고 아래에서 새로 채우게 한다.
    if st.session_state.get("_mypage_cached_user_id") != user["user_id"]:
        for stale_key in ("display_name", "predictions", "store_status", "store_snapshots", "owner_insight"):
            st.session_state.pop(stale_key, None)
        st.session_state["_mypage_cached_user_id"] = user["user_id"]
    ...
    # 지역과 같은 방식 + WHERE user_id=본인). get_favorite_regions()는 DB 연결이 안 됐으면
    # None, 연결은 됐는데 진짜 조회 이력이 없으면 빈 리스트를 반환한다 — "내 분석
    # 히스토리"와 똑같이 `or [더미...]`로 둘을 구분 안 해서, 방금 가입한 신규 회원도
    # 더미 지역이 마치 본인이 본 것처럼 보였다(2026-08-31 확인). None일 때만 더미로
    # 폴백한다.
    ...
    if db_regions is None:
        ...
        st.session_state.favorite_regions = db_regions

    # user_view_history(지도 클릭 → 개인별 조회 이력) 기준 "최근 관심있게 본 지역"
    # (유동인구/매장수/폐업률까지 같이 붙여서 판단 근거를 보여준다). 위와 같은 이유로
    # None(DB 연결 안 됨)일 때만 더미로 폴백한다.
    db_views = get_my_recent_views(user["user_id"])
    if db_views is None:
        st.session_state.my_recent_views = [
            {
    ...
    if "owner_insight" not in st.session_state and store_id and user["user_type"] == "owner":
        # "내 업종 생존점수/주변 경쟁 강도/업종전환 추천"을 카드 하나로 한눈에 보여주기
        # 위한 원본 데이터(2026-08-31 요청) — app.py 우리 가게 현황 패널과 같은 데이터.
        status = st.session_state.store_status
        st.session_state.owner_insight = {
            "prediction": get_prediction_for_store(store_id),
            "density": get_owner_competition_density(store_id, status.get("last_seen_snapshot")),
            "recommendations": get_industry_switch_recommendations(
                status.get("current_industry_code"), top_n=3
            ),
        }
...
# ────────────────────────────────────────────────
# 2-1. 내 업종 한눈에 보기 (생존점수 + 경쟁 강도 + 업종전환 추천) - 기존점주 전용
# ────────────────────────────────────────────────
def render_owner_insight_card():
    """생존점수/주변 경쟁 강도/업종전환 추천을 카드 하나로 한눈에 보여준다
    (2026-08-31 요청) — app.py "우리 가게 현황" 패널에 세 섹션으로 흩어져 있던
    것과 같은 데이터를 한 곳에 모았다."""
    insight = st.session_state.get("owner_insight")
    if insight is None:
        return

    with st.container(border=True):
        st.subheader("📊 내 업종 한눈에 보기")

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**내 업종 생존점수**")
            ...
            if pred:
                ...
                st.caption("아직 예측 점수가 없어요.")

        with col2:
            st.markdown("**주변 경쟁 강도 (반경 300m)**")
            density = insight["density"]
            if density:
                ratio = (
                    density["same_industry_count_300m"] / density["total_count_300m"] * 100
                    if density["total_count_300m"] else 0
                )
                st.metric("동일업종 비중", f"{ratio:.0f}%")
                st.caption(
                    f"동일업종 {density['same_industry_count_300m']}곳 / "
                    f"전체 {density['total_count_300m']}곳"
                )
            else:
                st.caption("경쟁 밀도 데이터가 없어요.")

        st.divider()
        st.markdown("**업종전환 추천 TOP 3**")
        recs = insight["recommendations"]
        if recs:
            rec_cols = st.columns(len(recs))
            for rec_col, r in zip(rec_cols, recs):
                with rec_col:
                    switch_score = round(r["wilson_score"] * 100)
                    st.markdown(f"**{r['industry_name']}**")
                    ui.grade_badge(switch_score)
                    caveat = f"표본 {r['sample_size']}건"
                    if r["low_confidence"]:
                        caveat = f"⚠️ {caveat} — 신뢰도 낮음"
                    if r["transition_count"] > 0:
                        caveat += f" · 실제 전환 {r['transition_count']}건"
                    st.caption(caveat)
...
def render_prediction_history():
    with st.container(border=True):
        ...
        if not predictions:
            user = st.session_state.user_info
            if user["user_type"] == "founder" and not user.get("store_id"):
                # 신규 입지 분석(예비창업자용) 기능 자체가 아직 없어서, 지금 유일하게
                # 쌓이는 이력인 "폐업 예측"은 본인 매장이 있어야만 생긴다 — 왜 비어있는지
                # 이유를 알려준다(2026-08-31 요청).
                st.info("실제 가게를 등록해야 분석 이력을 확인할 수 있어요.")
            else:
...
def main():
    ...
    # 기존점주(user_type=='owner')는 app.py 지도에서 자기 매장에 고정돼 다른 동을
    # 클릭해 둘러볼 수 없어서(OWNER 모드는 get_screen_mode()에서 그렇게 고정됨),
    # 지도 클릭 집계 기반 섹션("내가 관심있는 지역"/"자주 본 지역"/"최근 관심있게
    # 본 지역")은 카운트가 쌓일 수가 없어 의미가 없고, "내 매장 현황"도 app.py의
    # "우리 가게 현황" 패널과 중복이라 전부 같이 뺐다(2026-08-31 요청).
    #
    # 반면 예비창업자가 마이페이지에서 직접 가게를 등록(register_new_store())한
    # 경우는 user_type이 그대로 founder로 남아있어(2026-08-29, auth.py의 owner 로그인
    # 규칙(로그인 아이디=store_id, 고정 비밀번호)까지 맞추려면 범위 밖인 auth.py를
    # 건드려야 해서 회피) app.py에서도 OWNER 모드가 아니라 계속 지도를 자유롭게 클릭할
    # 수 있으므로, 이 경우엔 위 섹션들을 그대로 보여준다.
    user = st.session_state.user_info
    if user["user_type"] == "owner" and user.get("store_id"):
        render_owner_insight_card()
