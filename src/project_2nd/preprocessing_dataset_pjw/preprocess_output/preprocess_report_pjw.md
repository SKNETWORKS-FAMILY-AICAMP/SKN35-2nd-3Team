...
## 🆕 v5 (2026-08-26 재실행): 팀원분이 `gu_name`/생활인구 결측을 상류에서 직접 수정

팀원분(taemin1997)이 "결측 해소" 커밋을 main에 올림 — 저희가 v4에서 찾은 `gu_name` 결측 버그를 독립적으로 확인하고, 저희와 **똑같은 방식**(dong_code 앞 5자리 → 구 코드 매핑)으로 `build_modeling_dataset.py`에서 100% 채우도록 상류에서 고침. 한 걸음 더 나아가 생활인구 결측도 BallTree 최근접 이웃으로 근처 동의 값을 근사 대체하고, 어디가 근사치인지 표시하는 `population_is_proxied` 플래그 컬럼까지 추가함.

**의미**: 저희가 발견한 `gu_name` 이슈가 실제 문제였다는 게 팀원분 쪽에서도 독립적으로 재확인된 셈. 다만 이제 이 결측들이 상류에서 해결되기 때문에:
- 저희 쪽 `gu_name` 복구 로직(v4)은 더 이상 필요 없어졌지만(결측 0건), 안전망으로 코드는 그대로 둠
- "생활인구 결측 유지 vs gu 평균 대체" 비교 실험(아래 실험 A)은 이제 결측 자체가 거의 없어져서 더 이상 적용되지 않는 과거 실험으로 남김

**재실행 이슈**: `run_pipeline.sh`를 다시 돌리는 중 Windows 콘솔(cp949) 인코딩 때문에 팀원분 코드의 em dash(—) 출력에서 크래시 발생 — `PYTHONIOENCODING=utf-8` 환경변수로 우회(팀원 코드 자체는 수정 안 함, 환경 이슈일 뿐).

### 최종 검증 (v5, LightGBM 5-fold)

| | 새 원본(34컬럼) | 저희 정제본(37컬럼) |
|---|---|---|
| ROC-AUC | 0.747793 ± 0.0007 | **0.748440 ± 0.0009 (+0.00065)** |
| F1 | 0.279678 | 0.282237 |
| Accuracy | 0.906171 | 0.906280 |

v4(+0.00037) 대비 개선폭이 소폭 커짐 — censor/keyword 플래그 효과가 더 깨끗해진 데이터(gu_name/생활인구 결측 해소 후) 위에서 조금 더 뚜렷하게 드러난 것으로 추정. 5-fold 전부 일관된 방향은 v4와 동일하게 유지.
...
### 최종 검증 (새 데이터, LightGBM 5-fold) — ⚠️ v5에서 갱신됨, 위 v5 섹션 참고
...
✅ `gu_name`/생활인구 결측 상류 반영 요청 — v5에서 팀원분(taemin1997)이 "결측 해소" 커밋으로 직접 해결함 (위 v5 섹션 참고)
