...
| | 새 원본(33컬럼) | 저희 정제본(34컬럼, dong_code 제거 + 생활인구 결측 유지) |
|---|---|---|
| ROC-AUC | 0.747896 ± 0.0009 | 0.747853 ± 0.0008 |
| F1 | 0.280995 | 0.281517 |
| Accuracy | 0.906171 | 0.906243 |
...
- `store_age_months` 좌측 절단(left-censoring) 보완 — 첫 스냅샷(202312)에 이미 존재하던 매장은 실제 개업일을 알 수 없어 나이가 과소추정됨(팀원분 한계점 문서에 명시). "첫 관측 시점이 202312인지" 플래그 추가해서 검증 예정
- 트렌드 키워드 이진 플래그 테스트 — 기존 연속형 `keyword_growth_score` 외에 원시 매치 신호를 이진으로 넣었을 때 효과 있는지 검증 예정
